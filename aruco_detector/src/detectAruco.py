...
def loadImages(data):
     ...
     if ids is None: ##If no marker is detected
          ...
          if markerIndex is not None: #Only if id 0 is found
               ...
               if((abs(translationVectorInPixels[0]) > 0.1/thresholdFactor) and counter1%3 == 0):
                    msg.decision = 2
               ...
               elif((abs(translationVectorInPixels[1]) > 0.1/thresholdFactor) and counter1%3 == 0):
                    msg.decision = 1
               ...
               if not flag_zero_detected:
                    ...
                    centre_grp /= 4*len(ids)
                    lengthOfEdgeInPixel_grp = math.sqrt((corners[0][0][0][0] - corners[0][0][1][0])**2 + (corners[0][0][0][1] - corners[0][0][1][1])**2)
                    scaleFactor_grp = 0.13/lengthOfEdgeInPixel
                    translationVectorInPixels_grp = [(center[0] - 320),(center[1] - 240)]
                    translationVectorDrone_grp = [-scaleFactor*translationVectorInPixels[1],scaleFactor*translationVectorInPixels[0]]
     
                    if translationVectorInPixels_grp[1] > translationVectorInPixels_grp[0]:
                         msg.decision = 1
